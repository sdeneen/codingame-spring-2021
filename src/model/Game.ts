...
export default class Game {
     ...
     */
    getAllTrees = (): Tree[] => [...this.myPlayer.getTrees(), ...this.opponentPlayer.getTrees()];

    /**
     * Get the tree at the given cell index, if any exists
     */
    getTree = (cellIndex: number): Tree | null => this.myPlayer.cellIndexToTree[cellIndex] || this.opponentPlayer.cellIndexToTree[cellIndex] || null;

    /**
     * Get a deep copy of this object, replacing any provided fields
     */
    getModifiedDeepCopy = ({ day, nutrients, cells, myPlayer, opponentPlayer }: Partial<GameFields>): Game => {
        return new Game(
    ...
    equals = (other: Game): boolean => this.day === other.day
