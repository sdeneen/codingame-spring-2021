...
const getTreesWithSize = (trees: Tree[], size: number): number =>
  trees.filter((t) => t.size === size).length;

const calculateTreeActionCost = (
  ownedTrees: Tree[],
  ...
  switch (actionType) {
    ...
    case "GROW":
      switch (tree.size) {
        ...
        default:
          throw new Error(
            `We shouldn't be asking the cost of growing a tree of size ${tree.size}`
          );
      }
    case "SEED":
