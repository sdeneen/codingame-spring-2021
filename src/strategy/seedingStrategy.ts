    ...
    for (let i = 0; i < myTrees.length; i++) {
        ...
        if (!tree.isDormant) {
            if (calculateTreeActionCost(myTrees, "SEED", tree) === 0) {
                const freeCells = findCellsWithinDistance(game.cells, tree.cellIndex, tree.size).filter(cell => !cell.isOccupied() && cell.richness > UNUSABLE_RICHNESS);
                const freeSpaciousOrNutritiousCells = filterForSpaciousOrNutrientDenseCells(game, freeCells);
                freeSpaciousOrNutritiousCells.forEach(cell => seedActions.push(new Action("SEED", tree.cellIndex, cell.index)));
            }
        }
    }
...
const filterForSpaciousOrNutrientDenseCells = (game: Game, seedableCells: Cell[]): Cell[] => {
    return seedableCells.filter(seedableCell => {
        ...
        return numNeighbors < 2 || (seedableCell.richness === HIGH_RICHNESS && numNeighbors < 3);
    });
}
