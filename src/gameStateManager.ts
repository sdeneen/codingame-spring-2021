...
const determineGameStateAfterGrowAction = (currentState: Game, targetCellIndex: number): Game => {
    ...
    if (tree === null) {
        console.error(`Requested a GROW action on cell index ${targetCellIndex} which doesn't have a tree`);
        return currentState;
    }
    ...
    if (sunPoints < sunPointCost) {
        console.error(`Requested a GROW action on cell index ${targetCellIndex} which would cost ${sunPointCost} sun points but the player only has ${player.sunPoints}`);
        return currentState;
    }
